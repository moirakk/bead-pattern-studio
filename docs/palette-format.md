...
当前应用默认内置 `MARD 291 全色色卡`，由常见的 221 色加 P / Q / R / T / Y / ZG 扩展系列组成。用户也可切回 `MARD 221 常用版`。正式接店铺材料包、报价或库存时，仍可导入店铺或品牌提供的真实色卡 CSV 覆盖默认色卡。

导出的 PNG/PDF 会记录当前色卡来源。使用内置 MARD 291 或 MARD 221 时，会标注 HEX 为屏幕近似值，实物颜色以豆子批次为准。
