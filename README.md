...
拼豆创作平台原型：从“任意图片转拼豆图纸”开始，逐步扩展到图纸识别、个人作品集、社群分享、店铺色卡和材料包。当前版本是网页 MVP，已验证核心流程：上传图片、裁剪、像素化、限制成品尺寸/豆数、默认按 MARD 291 全色色卡匹配、可切换 MARD 221 常用版或导入店铺色卡 CSV、手工改单格、撤销/重做、统计用量，并导出带网格、坐标、色号和图例的 PNG/PDF 图纸。手机端还包含社区发现原型，可查看作品详情、收藏和复刻，并为本地作品保存包含昵称、说明与复刻权限的发布草稿。
...
- 成品宽高与自动比例
- 豆子数量预估
- 默认 MARD 291 全色色卡，可切换 MARD 221 常用版
- 店铺色卡 CSV 导入覆盖
- RGB 转 Lab 后做最近色匹配
- 色数上限控制
- 单格点击改色
