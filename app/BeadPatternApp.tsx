...
import { makePdfFromJpegPages, type PdfImagePage } from "@/lib/export/pdf";
...
export function BeadPatternApp() {
  ...
  /** Convert a finished A4 canvas to a lightweight PdfImagePage and release pixel memory. */
  function canvasToPage(canvas: HTMLCanvasElement): PdfImagePage {
  ...
  function makeA4SummaryPage(): PdfImagePage | null {
    if (!pattern) return null;
  ...
  function makeA4LegendPages(): PdfImagePage[] {
    if (!pattern) return [];
  ...
  function makeA4GridPages(): PdfImagePage[] {
    if (!pattern) return [];
    const pages: PdfImagePage[] = [];
  ...
  function exportPdf() {
    ...
    const pages = [summaryPage, ...legendPages, ...gridPages].filter((page): page is PdfImagePage => Boolean(page));
    if (!pages.length) return;
    const pdf = makePdfFromJpegPages(pages);
